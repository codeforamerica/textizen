  ...
  def html_histogram(histogram)
    ...
    if histogram
      ...
      histogram.slice(0,7).each do |h|
        result << "<tr style='font-size:#{h[1]/8.0}em' class='histogram-pair'><td class='histogram-word'>#{h[0]}</td><td class='histogram-separator'>:</td><td class='histogram-count'>#{h[1]}</td></tr>\n"
      end
      result << "</table>"
    end
    return raw result
